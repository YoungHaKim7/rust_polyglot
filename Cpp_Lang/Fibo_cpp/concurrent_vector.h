...
protected:
...
#if TBB_USE_ASSERT
    ~segment_t() {
      __TBB_ASSERT(array <= internal::vector_allocation_error_flag,
  ...
  void internal_grow(size_type start, size_type finish, size_type element_size,
  ...
  void __TBB_EXPORTED_METHOD internal_copy(const concurrent_vector_base_v3 &src,
                                           size_type element_size,
                                           internal_array_op2 copy);
  void __TBB_EXPORTED_METHOD
  internal_assign(const concurrent_vector_base_v3 &src, size_type element_size,
  ...
  void __TBB_EXPORTED_METHOD internal_resize(
      size_type n, size_type element_size, size_type max_size, const void *src,
      internal_array_op1 destroy, internal_array_op2 init);
  size_type __TBB_EXPORTED_METHOD internal_grow_to_at_least_with_result(
      size_type new_size, size_type element_size, internal_array_op2 init,
      const void *src);

  //! Deprecated entry point for backwards compatibility to TBB 2.1.
  void __TBB_EXPORTED_METHOD internal_grow_to_at_least(size_type new_size,
                                                       size_type element_size,
                                                       internal_array_op2 init,
                                                       const void *src);
...
template <typename Container, typename Value> class vector_iterator {
  ...
  friend vector_iterator<C, T> operator+(ptrdiff_t offset,
...
public: // workaround for MSVC
  ...
  vector_iterator(const Container &vector, size_t index, void *ptr = 0)
      : my_vector(const_cast<Container *>(&vector)), my_index(index),
...
public:
  ...
  vector_iterator(
      ...
      : my_vector(other.my_vector), my_index(other.my_index),
...
public:
  ...
  typedef
      typename internal::allocator_base<T, A>::allocator_type allocator_type;
  ...
  concurrent_vector(const concurrent_vector &vector,
    ...
    __TBB_CATCH(...) {
      ...
      internal_free_segments(reinterpret_cast<void **>(table),
                             internal_clear(&destroy_array), my_first_block);
      __TBB_RETHROW();
    }
  }
  ...
  //! Construction with initial size specified by argument n, initialization by
  //! copying of t, and given allocator instance
  concurrent_vector(size_type n, const_reference t,
                    const allocator_type &a = allocator_type())
      : internal::allocator_base<T, A>(a) {
    vector_allocator_ptr = &internal_allocator;
    __TBB_TRY {
      internal_resize(n, sizeof(T), max_size(), static_cast<const void *>(&t),
                      &destroy_array, &initialize_array_by);
    }
    __TBB_CATCH(...) {
      segment_t *table = my_segment;
      internal_free_segments(reinterpret_cast<void **>(table),
  ...
  concurrent_vector &operator=(const concurrent_vector<T, M> &vector) {
    if (static_cast<void *>(this) != static_cast<const void *>(&vector))
      internal_assign(vector.internal_vector_base(), sizeof(T), &destroy_array,
                      &assign_array, &copy_array);
    return *this;
  }
  ...
  size_type grow_by(size_type delta) {
    return delta ? internal_grow_by(delta, sizeof(T), &initialize_array, NULL)
                 : my_early_size;
  }
#else
  ...
  size_type grow_by(size_type delta, const_reference t) {
    return delta ? internal_grow_by(delta, sizeof(T), &initialize_array_by,
  ...
  iterator grow_by(size_type delta, const_reference t) {
    return iterator(
        *this, delta ? internal_grow_by(delta, sizeof(T), &initialize_array_by,
                                        static_cast<const void *>(&t))
                     : my_early_size);
  }
#endif
  ...
  /** The new elements are default constructed.  Blocks until all elements in
     ...
     constructed by other threads. */
  void grow_to_at_least(size_type n) {
    if (n)
      internal_grow_to_at_least_with_result(n, sizeof(T), &initialize_array,
                                            NULL);
  };
#else
  /** The new elements are default constructed.  Blocks until all elements in
     range [0..n) are allocated. May return while other elements are being
     constructed by other threads. Returns iterator that points to beginning of
     appended sequence. If no elements were appended, returns iterator pointing
     to nth element. */
  iterator grow_to_at_least(size_type n) {
    size_type m = 0;
    if (n) {
      m = internal_grow_to_at_least_with_result(n, sizeof(T), &initialize_array,
                                                NULL);
      if (m > n)
        m = n;
    }
  ...
  /** This method is thread-safe for concurrent reads, and also while growing
     the vector, as long as the calling thread has checked that index&lt;size().
   */
  ...
  //! Return false if vector is not empty or has elements under construction at
  //! least.
  ...
  /** Like most of the methods provided for STL compatibility, this method is
     *not* thread safe. The capacity afterwards may be bigger than the requested
     reservation. */
...
private:
  ...
  void internal_free_segments(void *table[], segment_index_t k,
                              segment_index_t first_block);
  ...
  void internal_assign_n(size_type n, const_pointer p) {
    internal_resize(n, sizeof(T), max_size(), static_cast<const void *>(p),
                    &destroy_array,
  ...
  static void __TBB_EXPORTED_FUNC initialize_array(void *begin, const void *,
                                                   size_type n);
  ...
  static void __TBB_EXPORTED_FUNC assign_array(void *dst, const void *src,
                                               size_type n);
  ...
  public:
    ...
    void init(const void *src) {
      for (; i < n; ++i)
        new (&array[i]) T(*static_cast<const T *>(src));
    }
    void copy(const void *src) {
      for (; i < n; ++i)
        new (&array[i]) T(static_cast<const T *>(src)[i]);
    }
    void assign(const void *src) {
      for (; i < n; ++i)
        array[i] = static_cast<const T *>(src)[i];
    }
...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4701) // potentially uninitialized local variable "old"
#endif
template <typename T, class A> void concurrent_vector<T, A>::shrink_to_fit() {
  ...
  __TBB_CATCH(...) {
    if (old.first_block) // free segment allocated for compacting. Only for
                         // support of exceptions in ctor of user T[ype]
...
void concurrent_vector<T, A>::internal_free_segments(
    void *table[], segment_index_t k, segment_index_t first_block) {
  // Free the arrays
  while (k > first_block) {
    --k;
    T *array = static_cast<T *>(table[k]);
    table[k] = NULL;
    if (array > internal::vector_allocation_error_flag) // check for correct
                                                        // segment pointer
...
T &concurrent_vector<T, A>::internal_subscript(size_type index) const {
  ...
  __TBB_ASSERT((segment_t *)my_segment != my_storage ||
  ...
  T *array =
      static_cast<T *>(tbb::internal::itt_hide_load_word(my_segment[k].array));
  __TBB_ASSERT(array != internal::vector_allocation_error_flag,
...
T &concurrent_vector<T, A>::internal_subscript_with_exceptions(
  ...
  if (array <= internal::vector_allocation_error_flag) // check for correct
                                                       // segment pointer
    internal::throw_exception(
        internal::eid_index_range_error); // throw std::range_error
...
template <typename T, class A>
void concurrent_vector<T, A>::initialize_array_by(void *begin, const void *src,
                                                  size_type n) {
  internal_loop_guide loop(n, begin);
  loop.init(src);
}
...
template <typename T, class A>
void concurrent_vector<T, A>::copy_array(void *dst, const void *src,
                                         size_type n) {
...
template <typename T, class A>
void concurrent_vector<T, A>::assign_array(void *dst, const void *src,
                                           size_type n) {
  internal_loop_guide loop(n, dst);
  loop.assign(src);
}
